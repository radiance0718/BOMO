...
def loop(scale, miu, timestep_respacing, save_dir):
    ...
    task_config['conditioning']['params']['miu'] = miu
    diffusion_config['timestep_respacing'] = timestep_respacing

    print(f"Processing {task_config['conditioning']['params']['scale']}_{task_config['conditioning']['params']['miu']}_{diffusion_config['timestep_respacing']}")
...
def main():
    scales = [0.5, 2, 5, 7, 10, 13, 15]
    mius = [0.01, 0.1, 0.3]
    timestep_respacing = 'ddim125'
    for scale in scales:
        for miu in mius:
            save_dir = f"./results/{scale}_{miu}_{timestep_respacing}"
            loop(scale, miu, timestep_respacing, save_dir)
